...
class BleManager private constructor(){
    ...
    /** 操作超时 */
    private var operateTimeout = DEFAULT_OPERATE_TIME
    /** 重试连接数 */
    private var reConnectCount = DEFAULT_CONNECT_RETRY_COUNT
    private var reConnectInterval = DEFAULT_CONNECT_RETRY_INTERVAL.toLong()
    private var splitWriteNum = DEFAULT_WRITE_DATA_SPLIT_COUNT
    /** 连接超时时间 */
    private var connectOverTime = DEFAULT_CONNECT_OVER_TIME.toLong()
    private var multipleBluetoothController: MultipleBluetoothController? = null
    ...
    fun isSupportBle(): Boolean {
        return  contextWeakReference.get()?.applicationContext?.packageManager?.hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)?:false
    }
    ...
    fun connect(bleDevice: BleDevice, bleGattCallback: BleGattCallback): BluetoothGatt? {
        ...
        if (!isBlueEnable()) {
           ("Bluetooth not enable!").logDebug(TAG)
            bleGattCallback.onConnectFail(bleDevice, Exception("Bluetooth not enable!"))
            return null
        }
        if (Looper.myLooper() == null || Looper.myLooper() != Looper.getMainLooper()) {
            ("Be careful: currentThread is not MainThread!").logError()
        }
        if(null != bleDevice.mDevice && null != multipleBluetoothController){
            val bleBluetooth: BleBluetooth = multipleBluetoothController!!.buildConnectingBle(bleDevice)
            val autoConnect: Boolean = bleScanRuleConfig.mAutoConnect
           return bleBluetooth.connect(bleDevice, autoConnect, bleGattCallback)
        }else{
            bleGattCallback.onConnectFail(bleDevice, Exception("Not Found Device Exception Occurred!"))
        }
    ...
    fun getConnectState(bleDevice: BleDevice): Int {
        return bleDevice.mDevice?.let {
            bluetoothManager?.getConnectionState(it,BluetoothProfile.GATT)?:BluetoothProfile.STATE_DISCONNECTED
        }?: BluetoothProfile.STATE_DISCONNECTED

    }
    fun getAllConnectedDevice(): List<BleDevice> {
        return  multipleBluetoothController?.let {
            it.getDeviceList()
        }?: listOf()
