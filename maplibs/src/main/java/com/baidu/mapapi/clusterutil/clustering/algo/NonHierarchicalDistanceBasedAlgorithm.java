...
public class NonHierarchicalDistanceBasedAlgorithm<T extends ClusterItem> implements Algorithm<T> {
   ...
   * Any modifications should be synchronized on mQuadTree.
   */
  private final PointQuadTree<QuadItem<T>> mQuadTree = new PointQuadTree<NonHierarchicalDistanceBasedAlgorithm.QuadItem<T>>(0, 1, 0, 1);
  ...
  @Override public Set<? extends Cluster<T>> getClusters(double zoom) {
    ...
    System.out.println("baidu =" + discreteZoom + "zoomSpecificSpan=" + zoomSpecificSpan);
    final Set<QuadItem<T>> visitedCandidates = new HashSet<NonHierarchicalDistanceBasedAlgorithm.QuadItem<T>>();
    final Set<Cluster<T>> results = new HashSet<Cluster<T>>();
    final Map<QuadItem<T>, Double> distanceToCluster = new HashMap<NonHierarchicalDistanceBasedAlgorithm.QuadItem<T>, Double>();
    final Map<QuadItem<T>, StaticCluster<T>> itemToCluster = new HashMap<NonHierarchicalDistanceBasedAlgorithm.QuadItem<T>, StaticCluster<T>>();

    synchronized (mQuadTree) {
      for (QuadItem<T> candidate : getClusteringItems(
          mQuadTree, discreteZoom)) {
  ...
  static class QuadItem<T extends ClusterItem> implements PointQuadTree.Item, Cluster<T> {
    ...
    @Override public Set<T> getItems() {
      return singletonSet;
    }
    ...
    @Override public boolean equals(Object other) {
      ...
      return ((QuadItem<?>) other).mClusterItem.equals(
          mClusterItem);
    }
  }
}
