...
abstract class AbstractBaiduMapMarkersFragmentDialog<T : IMarker> : BaseBdMapBottomSheetDialog() {
    ...
    override fun initView() {
        ...
        popView = rootView.findViewById(R.id.item_openmap_root)
        popView.gone()
        ...
        markerInfoViewPager2Adapter.iOperationListen = object : IOperationListion<T> {
            ...
            override fun onPathPlan(postion: Int, iMarkerModel: T) {
                onMarkerPathPlan(postion, iMarkerModel)
            }
            ...
            override fun onOtherOperation(postion: Int, iMarkerModel: T, view: View?) {
                onMarkerOtherOperation(postion, iMarkerModel, view)
            }
        }
        markerInfoViewPager2Adapter.loadImage = object : ILoadImage {
            override fun loadImageView(path: String, context: Context, imageView: ImageView) {
                loadMarkerImageView(path, context, imageView)
            }
        }
        ...
        bdMapManager.initMapMarkerConfig {
            if (supportMapCluster()) {
                ...
                bdMapManager.findMarkerDataAndIndexByMarker(it, dataQueue)?.let { pair ->
        ...
        try {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                val pathLatlngs = pathDataQueue.map {
                    pathIndex.add(if (it.isComplete()) {
                        0
                    } else {
                        1
                    })
    ...
    private var mPopupBottonWindow: QMUIPopup? = null
...
//                     it.dismiss()
//                 }else{
//                     it.showAtLocation(closeButton, Gravity.BOTTOM, 0, 0)
//                 }
//             }
            popView.visiable()
            //      mPopupBottonWindow?.show(closeButton)
    ...
    private fun initSelectedIndexViewDis(postion: Int) {
        markerInfoViewPager2Adapter.getItem(postion).apply {
            disFromPostion = bdMapManager.getMyLocationData()?.let { location ->
...
//                 isFocusable = true
//                 setBackgroundDrawable(ContextCompat.getDrawable(requireContext(),R.drawable.qmui_popup_bg))
