...
final class CameraConfigurationManager {
  ...
  private Point customResolution;
  private int   offsetHeight;//偏移高度
...
CameraConfigurationManager(Context context) {
    ...
    if(Build.VERSION.SDK_INT <17){
      ...
      display.getSize(offset);
      display.getRealSize(theScreenResolution);
      offsetHeight = Math.max(theScreenResolution.x,theScreenResolution.y)-Math.max(offset.x,offset.y);
  ...
  public void setCustomResolution(Point customResolution) {
...
//获取相机最适合的previewSize
  public   void   initCameraPreviewSizeValue(Camera.Parameters   parameters){
	  ...
	  cameraResolution = CameraConfigurationUtils.findBestPreviewSizeValue(parameters, screenResolution);
	  System.out.println("最佳尺寸："+cameraResolution.x+":"+cameraResolution.y);
